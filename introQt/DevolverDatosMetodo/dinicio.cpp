...
void DInicio::slotAbrirDialogo(){
	...
	//connect(dtrans, SIGNAL(senyalTransferenciaOrdenada(float,QString)),
	//	this, SLOT(slotListoTransDato(float,QString)));
...
void DInicio::slotListoTransDato(float cantidad, QString destino){


	QString res("Transferencia de ");
	res += QString::number(cantidad);
	res += " eurillos. Destino: ";
	res += destino;
	lbtransferencia->setText(res);

}
void DInicio::slotListoTrans(){

	//QString cuenta = dtrans->getCuenta();
	//float cant = dtrans->getCantidad();
	
	QString cuenta;
	float cant;
	bool aceptado;
	
	aceptado = dtrans->getCuentas(cant,cuenta);
	
	if (!aceptado) return:
	
	slotListoTransDato(cant,cuenta);
